export class ContactFileProcessor {
	...
	private replaceAutogeneratedMarkdownTag(lines: string[]): string[] {
		const startReplacementIndex = lines.findIndex((line) =>
			line.startsWith(this.autogeneratedMarkdownTag),
		);
		const endReplacementIndex = lines.findLastIndex((line) =>
			line.startsWith(this.autogeneratedMarkdownTag),
		);
		...
		lines.splice(
			...
			endReplacementIndex - startReplacementIndex + 1,
			this.markdown,
		);
		return lines;
	}
